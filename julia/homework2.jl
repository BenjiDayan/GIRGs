...
# ╔═╡ 45c0bd7c-ae17-4f82-ae67-84a49ff5beff
Gray(0.9)
...
# ╔═╡ 45bd5711-7caf-444e-beb8-d0adad5f74d9
firstindex(v)

# ╔═╡ 73e365be-eaae-4417-b1c2-f25bdedb316f
# ╠═╡ disabled = true
#=╠═╡
v2 = OffsetArray(v, -2); firstindex(v2)
  ╠═╡ =#

# ╔═╡ 92d8be21-9207-4dbe-8944-a99fb58b888b
false ? 3 : 2

# ╔═╡ 802bec56-ee09-11ea-043e-51cf1db02a34
function extend(v::AbstractVector, i)
	f, l = firstindex(v), lastindex(v)
	return i < f ? v[f] : (i > l ? v[l] : v[i]) 
end
...
# ╔═╡ a66c0f56-f23d-461f-b94a-7381cf660ada
mean(v)
...
# ╔═╡ 56e4202d-75f5-436a-b42a-47dea0363ba9
...
# ╔═╡ 4db12b77-ebbe-4862-b183-2dd4bb1aed24
vtemp = randn((5,5))

# ╔═╡ 4e304eae-f2f5-45dc-b670-e74f84a13c19
k = ones((3, 3)); k = k/*(size(k)...)

# ╔═╡ 0180e0b5-2e69-4a2d-b795-912808f56dba
function reflect(k)
	shape = size(k)
	return k[[lastindex(ax):-1:firstindex(ax) for ax in axes(k)]...]
end

# ╔═╡ be53220a-f1c9-4897-add6-eec8956c0a40
reflect(vtemp)

# ╔═╡ 4ba02eab-1fb7-4a6c-81a4-824739031c6b
5 ÷ 2
...
function extend(M::AbstractMatrix, i, j)
	...
	i2 = min(max(i, 1), num_rows)
	j2 = min(max(j, 1), num_columns)
	return M[i2, j2]
end
...
function convolve(v::AbstractVector, k)
	...
	l = l ÷ 2
	k = OffsetArray(k, -l:l)
	return [sum([extend(v, i-m) * k[m] for m in axes(k, 1)]) for i in 1:length(v)]
end
...
# ╔═╡ 476ff757-f01e-4b4f-b136-32b44f858fa3
[1 2; 3 4]

# ╔═╡ 425d36f2-143a-40fa-a563-af16a56ecb0d
K = k; ls = map(x->x÷2, size(K)); OffsetArray(K, [-l:l for l in ls]...)

# ╔═╡ e3184990-42d9-4922-9050-3971ffd00cfd
[(a, b) for (a, b) in Iterators.product(axes(K)...)]

# ╔═╡ 67121d3c-0993-4ea9-b5da-7329d8daea8b
collect(Iterators.product(axes(K)...))

# ╔═╡ 8b96e0bc-ee15-11ea-11cd-cfecea7075a0
function convolve(M::AbstractMatrix, K::AbstractMatrix)
	ls = map(x->x÷2, size(K))
	K = OffsetArray(K, [-l:l for l in ls]...)
	return [sum([extend(M, i-a, j-b) * K[a, b] for (a, b) in Iterators.product(axes(K)...)]) for i in axes(M, 1), j in axes(M, 2) ]
end
...
begin
	...
	function with_sobel_edge_detect(image)
		out_x = convolve(image, Gx)
		out_y = convolve(image, Gy)
		return 0.8*image + 0.2*sqrt.(out_x.^2 + out_y.^2)
	end
end
...
# ╔═╡ fad0dc4e-affa-4022-a356-844317ced375
sobel_raw_camera_data
