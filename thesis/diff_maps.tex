...
With GIRGs, our \q{manifold} is the original geometric space $\chi$, e.g. a torus or a cube.
...
The approach used by \cite{garcia2019mercator} to estimate node locations for the Hyperbolic Random Graph model uses the second \q{Heat Kernel} edge weighting variant of \cite{belkin2001laplacian}. This is specified as taking
\begin{equation}
...
% , and hence the stationary distribution has $\pi_u \propto \widetilde{d}_u$.
...
We wish to calculate the expected squared hop distance in the rescaled $\chi$-space, $E[r_{uv}^2 | w_u, u \to v]$, where $u \to v$ is the event that we transition from node $u$ to $v$ in one step of the random walk. First ingredients we need:
% We can also quantify the expected random step distance in $\chi$-space. If the starting state is node $u$ at timestep $0$, and we transition to node $v$ in the first hop (necessarily $v \sim u$ is a neighbour), then $r_{uv}$ is the distance travelled in $\chi$-space due to our transition, our \q{hop distance}.
\begin{align*}
...
We can see this by using the fact that the modified weighted degree is now $\widetilde{d}_u = \sum_{v \sim u} d_u^{-1} d_v^{-1} = d_u^{-1} \sum_{v \sim u} d_v^{-1}$, and hence the stationary distribution has $\pi_u \propto \widetilde{d}_u$. For graphs where most nodes have a decent number of neighbours spread across lower and higher degree nodes, then $\sum_{v \sim u} d_v^{-1} = \Theta(d_u)$, and so $\pi_u \propto d_u^{-1} d_u = 1$. A simplification is to say that a node $u$ in the GIRG with $50$ neighbours then has 
\begin{align}
  \widetilde{d}_u = \frac{1}{50} \sum_{v \sim u} d_v^{-1} \approx \sim  \frac{1}{50}
...
\begin{figure}
  ...
  \begin{figure}
    ...
    \caption{
      ...
      % 2d truncated diffusion maps plotted - each point is the embedding of a node in the graph. (a) and (b) are on a 2d GIRG, (c) and (d) on a real-world social network graph. The effect of the $\uniformify$ point rescaling procedure is shown, in (b) and (d).}
