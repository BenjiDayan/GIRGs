...
import os
os.environ['DATA_PATH'] = '/cluster/home/bdayan/girgs/FE_FB_Torus_nipstemp/'
...
socfb_graphs = df.loc[df.Model == 'real-world'].loc[df.Type == 'socfb'].sort_values('Graph')
...
if __name__ == '__main__':
    ...
    graph_dict = graph_dicts[-2]
    in_path = graph_dict["FullPath"]
    out_path = fe._stagepath + "results.csv"
    graph_type = graph_dict["Group"]


    g = nk.readGraph(in_path, nk.Format.EdgeListSpaceOne)
    g = fe.shrink_to_giant_component(g)
    info, model = fe.fit_ndgirg_general(6, utils.LCC, False, True)(g)

# sbatch --time=24:00:00 --ntasks=1 --cpus-per-task=12 --mem-per-cpu=2G --wrap="python do_feature_extract.py"
# sbatch --time=24:00:00 --ntasks=1 --cpus-per-task=16 --mem-per-cpu=4G --wrap="python do_feature_extract.py"